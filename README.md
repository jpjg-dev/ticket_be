...
관리자 계정을 포함한 모든 사용자는 결제 준비·상태 조회·취소 API에서 본인이 소유한 예매와 결제에만 접근할 수 있습니다.
...
| 외부 의존성 | Redis 캐시와 PG 승인·조회·취소 Circuit Breaker 상태·호출 결과 |
...
| PG Circuit Breaker | 최근 `10`회 중 최소 `5`회 집계, 실패율 `50%`, OPEN `30s` | 승인·조회·취소 회로를 분리하고 timeout·5xx·408·429만 장애율에 반영합니다. |
| Redis Circuit Breaker | 최근 `10`회 중 최소 `5`회 집계, 실패율 `50%`, OPEN `10s` | Redis 장애 중 반복 timeout을 줄이고 제한된 DB fallback 경로로 빠르게 전환합니다. |
...
| Redis와 PG에 용도별 Circuit Breaker를 적용했습니다. | Retry와 단일 공용 회로를 함께 적용하는 방식 | 장애 중 신규 호출을 빠르게 차단하되, 결제 중복 호출과 서로 다른 연산의 장애 전파를 피하기 위해서입니다. | OPEN 차단, HALF_OPEN 복구, PG 4xx 실패율 제외, 보정 배치 중단 테스트를 통과했습니다. |
...
| 결제 준비와 만료 경합 | 두 경로가 같은 예매 그룹 락과 동일한 락 순서를 사용 | `READY` 결제와 만료된 예매가 함께 남는 상태 불일치가 발생하지 않았습니다. |
