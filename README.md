...
- 인기 공연 E2E 시나리오에서 정상 경합 거부와 예상 밖 오류를 분리해서 측정했습니다.
...
- k6 결과는 로컬 통제 환경에서의 비교 수치이며, 운영 capacity claim으로 보지 않습니다.
- soldOut fast-path는 중심 메시지가 아니라 카탈로그 데이터와 예약 상태를 분리한 설계의 보조 최적화로 봅니다.
...
| 설계 결정 | 이유 | 확인한 내용 |
| --- | --- | --- |
| `ReservationGroup` 기준 예매 | 다중 좌석 예매와 결제 1건의 관계를 명확히 관리하기 위해서입니다. | 부분 성공 group `0`을 확인했습니다. |
| 좌석 ID 정렬 후 row lock | 같은 좌석 묶음에 대한 lock 획득 순서를 통일하기 위해서입니다. | 동일/겹치는 좌석 요청에서 하나의 group만 성공했습니다. |
| 결제와 만료 경합 구간 lock | 결제 승인과 예약 만료가 같은 group을 동시에 바꾸지 못하게 하기 위해서입니다. | 먼저 확정된 흐름만 최종 상태로 남았습니다. |
...
| 정상 경합 거부와 예상 밖 오류 분리 | 인기 공연에서는 실패 요청 자체가 장애가 아닐 수 있기 때문입니다. | k6 지표에서 expected reject와 unexpected error를 나눴습니다. |
...
| 겹치는 좌석 묶음 요청 | 일부 좌석이 겹치는 요청에서도 중복 active 좌석이 생기지 않습니다. |
| 다중 좌석 예매 | 선택 좌석 전체가 가능할 때만 group이 생성되고, 일부 좌석만 성공하지 않습니다. |
| 예약 만료 | 결제되지 않은 group은 `EXPIRED`, 좌석은 `AVAILABLE`, READY 결제는 `FAILED`로 정리됩니다. |
...
| Refresh Token 재발급 | 같은 Refresh Token 동시 재발급 요청 중 하나만 성공합니다. |

## 고부하 E2E 검증 결과

인기 공연 오픈 직후 시나리오는 공연 목록, 상세, 좌석 조회, 예약 생성, 결제 준비, Mock PG 승인, 결제 확정까지 이어지는 E2E 여정으로 측정했습니다.
...
| 트랜잭션 분리 | 만료 처리 write tx와 좌석 조회 분리 | `314.78 journeys/s` | `2.66s` | `1,108` | `1,000` | `0` |
| 상태 분리 보완 | availability 기반 매진 상태 처리 | `336.94 journeys/s` | `244ms` | `0` | `1,000` | `0` |
...
인증 흐름과 Cookie 기반 토큰 정책은 [docs/auth-flow-readme.md](docs/auth-flow-readme.md)에 정리했습니다.
...
`/seats`는 회차별 만료 처리를 먼저 수행합니다. 이후 availability 기준으로 매진이면 `{ scheduleId, soldOut: true, seats: [] }`를 반환합니다.
...
| 영역 | 사용 기술 | 사용 이유 |
| --- | --- | --- |
| Language | Java 21 | Spring Boot 기반 백엔드 구현에 사용했습니다. |
| Framework | Spring Boot 3.5.13, Spring Web | REST API와 계층형 애플리케이션 구성을 위해 사용했습니다. |
| Persistence | Spring Data JPA, PostgreSQL, Flyway | 상태 전이, row lock, 스키마 관리를 위해 사용했습니다. |
| Security | Spring Security, JWT, HttpOnly Cookie | Cookie 기반 인증과 권한 검증을 위해 사용했습니다. |
| Cache | Spring Cache, Caffeine | 공연 목록/상세 같은 카탈로그 조회 최적화에 사용했습니다. |
| API Docs | springdoc-openapi, Swagger UI | 운영 환경에서 API 확인이 가능하도록 사용했습니다. |
| Observability | Spring Actuator, Micrometer, Prometheus registry | 부하 테스트 중 connection pool과 애플리케이션 상태를 확인하기 위해 사용했습니다. |
| Test | JUnit 5, Spring Boot Test, Spring Security Test, Mockito | 상태 전이와 동시성 흐름 검증에 사용했습니다. |
| Performance | k6, Mock PG | 인기 공연 E2E 흐름을 외부 PG 부하 없이 검증하기 위해 사용했습니다. |
...
- 성능 테스트는 `dev,perf` profile을 기준으로 실행합니다.
