...
| 장애 처리 | 읽기는 제한된 DB fallback, 쓰기는 저장 생략 | 신규 등록·입장 처리를 차단하는 fail-closed |
| 영속성 | 같은 인스턴스이므로 AOF에 함께 기록 | AOF `everysec`로 재시작 복구 |

Redis 컨테이너 메모리는 `512MB`, dataset의 `maxmemory`는 `256MB`로 시작합니다. `maxmemory`를 프로세스 한도와 동일하게 잡지 않고 key 메타데이터, client/AOF buffer, allocator 단편화와 AOF rewrite를 위한 여유를 남깁니다.

Redis 전체 `maxmemory-policy`는 `noeviction`으로 둡니다. 메모리가 가득 찼을 때 캐시를 포함한 기존 key를 임의 삭제하지 않고 신규 쓰기를 명시적으로 실패시켜, 대기 순서나 입장 권한이 조용히 사라지는 상황을 막습니다. 대신 캐시는 TTL과 크기를 제한하고 메모리 사용률·쓰기 거부를 관측해야 합니다.

AOF는 `appendfsync everysec`로 설정하고 `/data`를 Docker volume에 연결합니다. Redis 재시작 시 기록된 쓰기 명령을 재생하지만 최근 약 1초 내외의 변경은 유실될 수 있습니다. AOF는 재시작 복구 수단이며 복제나 고가용성을 대신하지 않습니다.

단일 인스턴스는 운영 구성이 단순한 대신 cache와 상태가 메모리·장애 지점을 공유합니다. 캐시가 상태 쓰기를 압박하거나 역할별 확장·복구 기준이 달라지는 시점에 물리 인스턴스 분리를 다시 검토합니다.
...
읽기 회로 OPEN은 즉시 `EventCacheAccessException`으로 변환되어 제한 DB fallback 정책으로 전달됩니다. 쓰기 회로 OPEN은 캐시 저장이나 refresh lock만 차단하며 정상적인 기존 cache 조회까지 막지 않습니다. Circuit Breaker는 DB fallback 상한을 대신하지 않습니다. 회로는 반복 timeout을 줄이고, 세마포어와 key별 단일 로더는 Redis가 없을 때 DB로 전달되는 요청 수를 제한합니다.
...
| 정책 | 얻는 것 | 비용·손실 | 현재 판단 |
| --- | --- | --- | --- |
| 단일 Redis | 컨테이너·모니터링·운영 비용 절감 | cache와 상태가 메모리·장애 지점을 공유 | 현재 규모에서는 단순성을 우선하고 분리 조건을 관측 |
| `noeviction` | 중요한 상태 key의 임의 제거 방지 | 메모리 한도에서 신규 cache·상태 쓰기 실패 | 조용한 상태 유실보다 명시적 실패를 우선해 채택 |
| AOF `everysec` | 재시작 후 상태 복구 | 디스크 I/O와 최근 약 1초 내외 유실 가능성 | 성능과 복구 가능성의 균형으로 채택 |
| 읽기·쓰기 회로 분리 | 쓰기 장애 중 기존 읽기 유지 | 회로와 지표 수 증가 | 복구 행동이 다른 연산의 장애 전파를 막기 위해 채택 |
...
운영 Redis와 분리한 Redis 7.4 컨테이너에 컨테이너 메모리 `512MB`, `maxmemory=256MB`, `noeviction`, AOF `everysec`를 적용했습니다.

| 검증 | 결과 |
| --- | --- |
| AOF 비정상 재시작 | marker 저장 후 2초 대기하고 `SIGKILL`한 뒤 동일 volume으로 재시작했을 때 marker가 복구됐습니다. |
| 메모리 포화 | 1MB 값 `202`개와 작은 값을 추가해 `used_memory=256MB`까지 채웠습니다. |
| 신규 쓰기 | 포화 상태의 4KB 신규 쓰기는 `OOM command not allowed`로 거부됐습니다. |
...
- 분산락 도입 조건, TTL, 소유권 token과 fencing 정책 검토
