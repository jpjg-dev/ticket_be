...
- [ ] `ReservationGroup` 상태 모델 추가
  - 현재 group은 예매 묶음의 최상위 객체이지만 `expiresAt`만 있고 상태가 없어, 자식 `Reservation` 상태를 조회해 group 상태를 추론해야 한다.
  - `PENDING`, `CONFIRMED`, `CANCELED`, `EXPIRED` 상태를 group에 추가하고 마이페이지/운영 조회의 기준 상태로 사용한다.
  - 결제 승인, 결제 취소, 예약 만료 시 group과 child reservation 상태는 같은 트랜잭션에서 함께 전이하도록 정합성 규칙을 정의한다.
  - group 상태 도입 시 `Reservation.expiresAt`을 유지할지, group 단일 만료 기준으로 단순화할지 함께 판단한다.
