...
- [ ] 좌석 조회 연계 수동 만료 처리 최적화
  - 현재 좌석 조회는 해당 회차의 만료 group을 즉시 정리한 뒤 최신 좌석 상태를 반환한다.
  - 혼합 Spike 3회 중앙값 기준 수동 만료 포함 조건은 처리량 `606.20 req/s`, 전체 p95 `3.31s`였고, 만료 제외 조건은 처리량 `801.51 req/s`, 전체 p95 `1.90s`였다.
  - 조회 연계 수동 만료는 병목 비용이 있지만 제거하지 않는다. 스케줄러 주기 사이에 만료 좌석이 판매 불가 상태로 남는 문제를 줄이고 좌석 상태 정합성과 즉시 재판매 가능성을 보장하는 것이 우선이다.
  - 정합성 정책을 유지하면서 회차별 만료 대상 조회 범위 축소, 조건부 상태 전이, 한 요청에서 처리할 만료 group 수 제한, 스케줄러 주기 조정을 검토한다.
- [ ] 좌석 선점 조건부 update 전환 검토
  - 현재 예약 생성은 `Seat` row를 비관적 락으로 조회한 뒤 `Seat.hold()`로 `AVAILABLE -> HELD` 상태 전이를 수행한다.
  - 조건부 update는 `where status = 'AVAILABLE'` 조건과 영향 row 수로 선점 성공 여부를 판단할 수 있어 고경합 실패 요청을 더 빨리 탈락시킬 수 있다.
  - 다만 이 방식은 `Seat.hold()` 도메인 메서드를 우회하고 상태 전이 정책을 SQL 조건으로 분산시키므로, 현재 구조에서는 도메인 명확성과 정합성 설명력이 약해진다.
  - 인기 공연 E2E arrival-rate 최초 측정에서 예약 생성 p95는 `28.63ms`였고, 좌석 조회 p95는 `3.16s`, 전체 여정 p95는 `15.60s`였다. 따라서 현재 병목은 좌석 선점 update 방식보다 조회/만료 처리 경로에 가깝다.
  - 후속 측정에서 예약 생성이 실제 병목으로 확인되면 `NOWAIT`, 짧은 lock timeout, 조건부 update를 같은 E2E 조건에서 비교한다.
...
- [ ] Python 기반 컨테이너 헬스체크 스크립트 검토
  - 배포 후 backend/frontend/nginx/postgres 컨테이너 상태와 주요 HTTP endpoint 응답을 한 번에 확인하는 스크립트를 검토한다.
  - 단순 `docker ps` 확인이 아니라 HTTP 상태 코드, 응답 시간, 컨테이너 재시작 여부까지 확인할 수 있게 한다.
  - GitHub Actions 배포 후 smoke test 단계나 GCP VM 수동 점검용으로 사용할 수 있다.
