...
- 따라서 대표 수치는 운영 최대 TPS 보장이 아니라 개선 전후 상대 비교와 정합성 유지 여부를 보여주는 값입니다.
...
| 인기 공연 E2E | 목록, 상세, 좌석, 예약, 결제 전체 여정 | 오픈 직후 포화 상황에서 조회 성능과 정합성 확인 | TPS, p95, dropped iteration, 완료 결제, DB 사후 검증 |
...
- 전체 여정 TPS는 완료 결제뿐 아니라 정상 경합/매진 거부까지 포함한 시스템 처리량입니다.
- 결제 TPS는 좌석 수 `1,000`개에 의해 상한이 생기므로 전체 여정 TPS와 분리해서 봅니다.
- availability 분리 후 네트워크 수신량은 `1.6GB -> 206MB`로 감소했습니다.
- 이 감소는 핵심 목표가 아니라, 카탈로그 데이터와 예약 상태를 분리한 결과로 얻은 부가 효과입니다.
