 ...
 */
record PgCancelState(String paymentKey, String status, String currency) {

    static PgCancelState from(TossCancelResponse response) {
        return new PgCancelState(response.paymentKey(), response.status(), response.currency());
    }

    static PgCancelState from(TossPaymentLookupResponse response) {
        return new PgCancelState(response.paymentKey(), response.status(), response.currency());
    }
}
