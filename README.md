...
| Redis Circuit Breaker | 읽기·쓰기 각각 최근 `10`회 중 최소 `5`회 집계, 실패율 `50%`, OPEN `10s` | 쓰기 실패가 정상 읽기까지 차단하지 않게 분리하고, 읽기 장애만 제한된 DB fallback 경로로 전환합니다. |
...
├── featureflag          # 운영 기능 전환 제어
│   ├── presentation     # 관리자 Feature Flag API
│   ├── application      # 대기열 모드 조회/변경, 캐시 갱신
│   ├── domain           # QueueFeatureFlag, QueueMode, Repository
...
환경 파일의 키셋은 값 없이 비교할 수 있습니다.

```powershell
.\scripts\Test-EnvKeyset.ps1 -EnvFile .env.dev
```
