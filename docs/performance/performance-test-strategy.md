...
| 결제 승인/취소 | `POST /payments/confirm`, `POST /payments/{id}/cancel` | 중복 요청에서 멱등키와 상태 재확인으로 최종 상태 수렴 | 최종 상태 일관성, 취소 PG Mock 호출 1회 |
...
캐시·트랜잭션 분리·SoldOut 정책 도입 전 커밋을 `2 vCPU` 제한 환경에서 다시 실행하고 Docker stats, Actuator, PostgreSQL wait, JFR을 함께 수집했습니다.

- backend CPU는 평균 `143.05%`, 최대 `178.29%`였고 system CPU는 평균 `91.2%`였습니다.
- PostgreSQL CPU는 평균 `24.60%`였으며 수집 구간에서 lock/I/O wait는 관찰되지 않았습니다.
- Hikari active는 `10`, pending은 최대 `168`이었습니다.
- JFR에서는 PostgreSQL 튜플 수신, Hibernate 엔티티 관리, JSON 직렬화가 주요 실행·할당 지점이었고 GC pause 총합은 `943ms`였습니다.

제한 자원 자체가 직접적인 포화 지점이었지만, 이를 유발한 비용은 초기 애플리케이션의 반복 조회·엔티티 구성·응답 직렬화였습니다. 커넥션 대기는 독립 원인보다 CPU 포화 이후 발생한 2차 대기로 판단했습니다.

상세 조건과 계측 한계는 [초기 E2E 병목 자원 분석](./issue-49-initial-bottleneck-resource-analysis.md)에 정리했습니다.
...
- 동일 `orderId` 결제 승인 동시 요청은 같은 멱등키로 PG confirm을 호출하고 내부 상태는 확정 상태로 수렴했습니다.
...
입장 스케줄러는 `15명 / 1초`로 설정했습니다. `fixedDelay`에는 이전 batch 실행시간이 더해지므로 TTL 계산에는 명목 처리율보다 낮은 최소 입장률 `10명/s`를 사용합니다.

```text
entry TTL > max expected backlog / minimum admission rate + safety margin
30분 > 10,000명 / 10명/s + 5분
```

잘못된 조합은 설정 바인딩 단계에서 기동을 거부합니다. Redis 통합 테스트에서는 10,000개 대기 토큰을 batch 단위로 모두 승격하고 마지막 토큰까지 `ADMITTED`, 유실 `0`을 확인했습니다.
...
- 실패 원인 진단 시 `LOG_UNEXPECTED=true`를 지정하면 사유, HTTP 상태, k6 오류 코드, 실행 경과 시간, 검증된 형식의 오류 코드와 traceId를 기록합니다. 응답 본문 전체와 Cookie는 기록하지 않습니다. k6 summary는 사유별 태그를 보존하지 않으므로 console 로그와 backend 로그를 컨테이너 재생성 전에 별도로 보관합니다.
- 종료 시점의 `status=0` 진단 로그와 summary에 집계된 unexpected 건수를 동일시하지 않습니다. 강제 중단된 요청의 로그·메트릭 반영 시점이 다를 수 있으므로 HTTP 응답 오류와 전송 중단을 나눠 분석합니다.
