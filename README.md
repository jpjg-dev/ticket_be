...
| 개선 대상 | 기준 조건 | 개선 전 | 개선 후 | 의미 |
| --- | --- | ---: | ---: | --- |
| 인기 공연 전체 여정 | 좌석 `1,000`개 매진 시나리오 | RPS `223.06`, p95 `15.60s`, dropped `5,694` | RPS `336.94`, p95 `244ms`, dropped `0` | 매진 이후 반복 좌석 조회를 줄이면서 정합성을 유지했습니다. |
| 마이페이지 N+1 | group `100`, `10 VU / 30s` | p95 `202.75ms`, 처리량 `60.73 req/s` | p95 `17.52ms`, 처리량 `663.46 req/s` | 같은 응답 구조를 유지하면서 반복 조회와 중복 DTO 조립 비용을 줄였습니다. |
...
| 좌석 조회 응답 크기가 병목입니다. | SoldOut 정책 적용 후 재측정 | 응답 크기 감소 효과는 있었지만, 핵심은 payload 축소가 아니라 매진 확정 이후 반복 좌석 조회를 차단한 점입니다. | 보조 효과로 정리했습니다. |
