...
| 전체 여정 TPS | `223.06 journeys/s` | `311.46 journeys/s` | `314.78 journeys/s` | `336.94 journeys/s` |
| 전체 여정 TPS 초기 대비 | - | `+39.63%` | `+41.12%` | `+51.05%` |
| 전체 여정 TPS 직전 대비 | - | `+39.63%` | `+1.07%` | `+7.04%` |
...
| 완료 결제 처리량 초기 대비 | - | `+13.43%` | `+14.65%` | `+16.29%` |
...
| 완료 결제 | `1,000` | `1,000` |
| 완료 결제 처리량 | `19.92 payments/s` | `19.92 payments/s` |
...
- 전체 여정 TPS는 `223.06 -> 336.94 journeys/s`로 `+51.05%` 개선됐다. 이 값은 완료 결제뿐 아니라 정상 경합/매진 거부까지 포함한 시스템 처리량이다.
- 완료 결제 처리량은 `17.13 -> 19.92 payments/s`로 `+16.29%` 개선됐다. 단, 좌석 수가 `1,000`개이므로 결제 TPS는 매진 이후 상한이 생긴다.
- 완료 iteration은 `11,153 -> 15,573 -> 15,739 -> 16,847`으로 늘었지만 좌석 수는 동일하게 `1,000`개이므로, 추가 완료분은 대부분 매진/경합에 따른 정상 거부로 집계됐다.
