...
현재는 `RestClient`가 적합하지만,
다음과 같은 상황이 생기면 다른 선택지를 다시 검토할 수 있다.

- 외부 PG, 알림, 정산 등 호출 대상이 크게 늘어나는 경우
- 외부 API 계약을 인터페이스 단위로 강하게 관리해야 하는 경우
- 비동기 대량 호출이 핵심 요구사항이 되는 경우
- 장애 대응, 재시도, 회로 차단 패턴을 본격적으로 도입하는 경우

이 경우 후보는 다음과 같다.

- OpenFeign: 외부 API가 많아지고 선언형 클라이언트가 필요할 때
- WebClient: 비동기/논블로킹 호출이 핵심이 될 때
- Resilience4j 조합: 재시도, timeout, circuit breaker가 필요할 때
