...
- 재발급은 Refresh Token Cookie를 검증하고, 기존 Refresh Token을 조건부 update로 한 번만 소비한 뒤 새 토큰 쌍을 발급한다.
- 조건부 update는 `jti`, `userId`, token hash가 일치하고 `revoked_at is null`, `expires_at > now`인 row만 `revoked_at`, `last_used_at`을 갱신한다.
- 영향 row가 `1`이면 최초 소비 요청으로 판단하고, `0`이면 이미 소비됐거나 유효하지 않은 토큰으로 판단해 `401`로 거부한다.
- 같은 사용자의 서로 다른 Refresh Token은 `jti`가 다르므로 기기별 세션처럼 독립적으로 재발급할 수 있다.
